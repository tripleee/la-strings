...
#define FrStoreLong(value, location) \
   __asm__ ("xchg %h0,%b0\n\t" \
	    "rol $16,%0\n\t" \
	    "xchg %h0,%b0" : "=q" (*(uint32_t*)(location)) : "0" (value) )
...
#define FrStore64(value, location) \
   __asm__ ("bswapq %0" : "=q" (*(uint64_t*)(location)) : "0" (value) )
#else
...
inline uint32_t FrLoadLong(const void *location)
{
   uint32_t result ;
   __asm__("movl %1,%0\n\t"
	   "bswap %0" : "=q" (result) : "g" (*(uint32_t*)(location))) ;
   return result ;
}
#elif defined(__GNUC__) && defined(__386__)
inline uint32_t FrLoadLong(const void *location)
...
inline uint64_t FrLoad64(const void *buffer)
{
   ...
   uint64_t result ;
   __asm__("movq %1,%0\n\t"
	   "bswap %0" : "=q" (result) : "g" (*(uint64_t*)(buffer))) ;
   return result ;
#else
