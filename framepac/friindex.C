...
//      virtual bool prefixMatch(const char *nameprefix,int len) const ;
      ...
      void flush(FrHashTable *cache, int fd) ;
   } ;
...
class FrIIEntryBitmap : public FrInvIndexEntry
   {
   private:
      static FrAllocator allocator ;
      char bitmap[DiskIndexBlockBitmap_SIZE] ;
...
class FrIIEntryIDList : public FrInvIndexEntry
   {
   private:
      static FrAllocator allocator ;
      int32_t IDs[DiskIndexBlockIDList_SIZE] ;
   public:
      void *operator new(size_t) { return allocator.allocate() ; }
      void operator delete(void *blk) { allocator.release(blk) ; }
      FrIIEntryIDList() ;
      virtual ~FrIIEntryIDList() ;
      static void compact(FrMemFooter *pages, FrFreeList **freelist,
			  FrMemFooter **freed) ;
   } ;
...
FrAllocator FrIIEntryBitmap::allocator("InvIndex Bitmap",
				       sizeof(FrIIEntryBitmap),
				       &FrIIEntryBitmap::compact) ;
FrAllocator FrIIEntryIDList::allocator("InvIndex IDList",
				       sizeof(FrIIEntryIDList),
				       &FrIIEntryIDList::compact) ;
...
/************************************************************************/
/*	Methods for class FrIIEntryIDList				*/
/************************************************************************/

void FrIIEntryIDList::compact(FrMemFooter *pages, FrFreeList **freelist,
			      FrMemFooter **freed)
{
   (void)pages ; (void)freelist ; (void)freed ;
...
{
   FrInvIndexEntry key(blocknum) ;
   FrInvIndexEntry *entry = (FrInvIndexEntry*)cache->lookup(&key) ;
   if (entry)
...
static bool flush_block(FrHashEntry *ent, va_list args)
{
   FrInvIndexEntry *entry = (FrInvIndexEntry*)ent ;
   FrVarArg(FrHashTable *,cache) ;
   FrVarArg(int,fd) ;
   if (entry->isDirty())
      {
	(void)fd ; (void)cache; //!!!
...
void FrInvertedIndex::flush()
{
   if (cache)
      {
      cache->doHashEntries(flush_block,cache,fd) ;
