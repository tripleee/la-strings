...
/*  LastEdit: 23may12							*/
/*									*/
/*  (c) Copyright 1994,1995,1996,1997,1998,2000,2001,2009,2012		*/
/*		 Ralf Brown						*/
...
class FrSymbol : public FrAtom
   {
   private:
      FrSymbol *m_next ;		// next symbol in same hash bucket
      ...
      char      m_name[1] ;
   private: // methods
      FrSymbol *next() const { return m_next ; }
	 ...
	 theDemons = d ;
#else
	 (void)d ; // keep compiler happy
#endif
	 }
      void *operator new(size_t size,void *where)
	  { (void)size; return where ; }
      FrSymbol(const char *symname,int len, FrSymbol *nxt = 0)
...
//				     const FrSymbol *facet) const ;
   ...
   //friends
      ...
      friend bool FrSymbolTable_insert(const char *symname, size_t len,
					 FrSymbol *&,FrSymbol *,FrSymbol *&);
      friend FrSymbol *new_FrSymbol(const char *name, size_t len) ;
      friend bool FrSymbolTable_iterate(FrSymbol *node, FrIteratorFunc func,
				   va_list args) ;
      friend bool FrSymbolTable_iterate_frame(FrSymbol *node, FrIteratorFunc func,
						va_list args) ;
   } ;
