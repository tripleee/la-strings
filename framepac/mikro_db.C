...
#elif defined(__SUNOS__) || defined(__SOLARIS__) || defined(unix)
#  include <unistd.h>
...
static int read_index_byname_entry(Fr_FILE *fp, DBFILE *db)
{
   ...
   {
   HashEntryVFrame new_ent(sym,offset,idxpos,frameID) ;
   new_ent.deleted = deleted ;
   entry = db->index->add(&new_ent) ;
#ifdef FrFRAME_ID
...
int load_byname_index(DBFILE *db)
{
   ...
   db->index = new FrHashTable(2) ;
   db->index->expand((int)num_entries+100) ;
   db->entrycount = 0 ;
...
{
   HashEntryVFrame key(frname) ;
   return (HashEntryVFrame *)db->index->lookup(&key) ;
}
