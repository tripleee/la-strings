...
/*  LastEdit: 04nov09							*/
/*									*/
/*  (c) Copyright 1994,1995,1996,1997,2000,2001,2009 Ralf Brown		*/
...
bool VFrameInfoFile::isFrame(const FrSymbol *name) const
{
   if (hash)
      {
      HashEntryVFrame key(name) ;
      HashEntryVFrame *ent = (HashEntryVFrame*)hash->lookup(&key) ;
      return (ent && !ent->deleted) ;
      }
   else
      return false ;
}
...
bool VFrameInfoFile::isLocked(const FrSymbol *frame) const
{
   if (hash)
      {
      HashEntryVFrame key(frame) ;
      HashEntryVFrame *entry = (HashEntryVFrame *)hash->lookup(&key) ;

      return entry ? (bool)entry->locked : false ;
      }
   else
      return false ;
}
...
VFrame *VFrameInfoFile::retrieveFrame(const FrSymbol *name) const
{
   ...
   if (db)
      ...
      char *rep ;
      HashEntryVFrame key(name) ;
      HashEntryVFrame *entry = (HashEntryVFrame *)db->index->lookup(&key) ;
...
static bool sync_frame(FrHashEntry *ent, va_list args)
{
   FrVarArg(DBFILE *,db) ;
   ...
   if (force ||
       (((HashEntryVFrame*)ent)->frameName()->symbolFrame() &&
        ((HashEntryVFrame*)ent)->frameName()->symbolFrame()->dirtyFrame())
      )
      {
      if (hook)
         hook(((HashEntryVFrame*)ent)->frameName()) ;
      int result = update_database_record(ent,db,force,synch) ;
      if (result)
         ((HashEntryVFrame*)ent)->frameName()->symbolFrame()->markDirty(false);
      return result != false ;	
      }
   return true ;
}
...
int VFrameInfoFile::syncFrames(frame_update_hookfunc *hook)
{
   ...
   if (hash->doHashEntries(sync_frame,db,false,false,hook) &&
...
{
   HashEntryVFrame key(name) ;
   HashEntryVFrame *entry ;

   if (db)
      {
      entry = (HashEntryVFrame *)db->index->add(&key) ;
      if (entry)
	 {
	 entry->deleted = false ;
...
int VFrameInfoFile::deleteFrame(const FrSymbol *name, bool upd_backstore)
{
   ...
   if (db)
      {
      entry = (HashEntryVFrame *)db->index->lookup(&key) ;
      if (entry)
	 {
         entry->deleted = true ;
